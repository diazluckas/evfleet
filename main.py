...
""" 
DEFINE SOC POLICY LIST
"""
soc_policy = [(5, 100), (5, 95), (5, 85),
              (10, 100), (10, 95), (10, 85),
              (20, 100), (20, 95), (20, 85), 
              (50, 100), (10, 60), (20, 70), 
              (30, 80), (10, 70), (30, 90)]

#soc_policy = [(0, 25), (25, 50), (50, 75),
#              (75, 100), (0, 50), (25, 75),
#              (50, 100), (25, 100), (0, 75), (0, 100)]
...
# cicle options
parser.add_argument('--cicle', type=str, help='Defining the usage cycle to degradate', default='energy')

# degradation model options
parser.add_argument('--model', type=str, help='Define the degradation model (aramis/severity)', default='aramis')
...
if __name__ == '__main__':
    ...
    """
    MODIFICACION LUCKAS: se agrega una condicion para correr degradacion-openloop
    """
    if args.stage == 4:
        print(f'Considering the following policy:\n {args.soc_policy}')
        input('Press any key to continue... (ctrl+Z to finish process)')
        ...
        source_folder = args.target_folder.parent
        simulations_folder = Path(args.target_folder.parent, 'simulations', 'simulations_OpenLoop_degradation')
        Online.online_operation_degradation_open_loop(args.target_folder, source_folder, simulations_folder, on_hp, eta_table,  ev_type, fleet_type, edge_type, network_type, 
                                            keep_times=args.keep_times,
                                            sample_time=args.sample_time, std_factor=args.std_factor, policy=args.soc_policy,
                                            degrade_until=0.8, cicle=args.cicle, set_model=args.model, 
                                            repetitions=args.preop_repetitions, additional_vehicles=args.additional_vehicles, 
                                            fill_up_to=args.fill_up_to)

    elif args.stage == 5:
        print(f'Considering the following SOC policies:\n {soc_policy}')
        input('Press ENTER to continue... (ctrl+Z to finish process)')
        ...
        pool_size = len(soc_policy)

        args = [(args.target_folder, source_folder, simulations_folder, on_hp, 
                eta_table,  ev_type, fleet_type, edge_type, network_type, args.keep_times,
                args.sample_time, args.std_factor, policy,
                0.8, args.cicle, args.model, args.preop_repetitions, args.additional_vehicles, 
                args.fill_up_to) for policy in soc_policy]
        pool = multiprocessing.Pool(processes=pool_size)
        pool.starmap_async(Online.online_operation_degradation_open_loop, args)
        pool.close()
        pool.join()
