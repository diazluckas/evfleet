...
def pre_operation(instance_filepath: Path, hp: AlphaGA_HyperParameters, soc_policy: Tuple[float, float] = None,
                  ...
                  edge_type: Union[Type[Network.Edge.DynamicEdge], str] = None, openloop = False):
    # Setup directories
    if not openloop:
        if method_name:
        ...
        multiple_results_folder = Path(instance_folder.parent, 'day_preOperation')
    ...
    for _ in range(repetitions):
        ...
        if not openloop:
            ...
            execution_folder = Path(multiple_results_folder, 'iteration'+str(_))
    ...
    return results, multiple_results_folder, s
...
def folder_pre_operation(folder_path: Path, hp: AlphaGA_HyperParameters, soc_policy: Tuple[float, float] = None,
                         ...
                         sat_prob_sample_time: float = 120, cs_capacities: int = None, parallel=True, openloop=False):
    ...
    other_args = (hp, soc_policy, additional_vehicles, fill_up_to, external_individual, sat_prob_sample_time,
                  cs_capacities, method_name, repetitions, fleet_type, ev_type, network_type, edge_type, openloop)
    if parallel:
        ...
        for instance in instances:
            results, multiple_results_folder, s = pre_operation(instance, hp, soc_policy, additional_vehicles, fill_up_to, external_individual,
                          sat_prob_sample_time, cs_capacities, method_name, repetitions, fleet_type,
                          ev_type, network_type, edge_type, openloop)
            return s
