...
def severity(socl, soch, current_charge, current_discharge, battery_capacity):

    # Load severity factor data
    d = os.getcwd()
    path = Path(d, 'data/severity-25c.csv')
    severity_df = pd.read_csv(path, header=None, names=['DOD', 'Severity'])
    # Useful Life
    L = 676067.9493
    L_eq = L * 5
    # The model is applied
    dod = soch - socl
    val, idx = find_nearest(severity_df['DOD'], dod)
    severity_factor = (severity_df['Severity'].iloc[idx])/2
    current = np.abs(current_charge) + np.abs((current_discharge/3600000)) #kWh
    Aheff = ((current*1000)/350) * severity_factor #Ah
    capacity_res = ((battery_capacity*1000)/350)-(Aheff/L_eq) #Ah
    return (capacity_res/1000)*350*3600000, severity_factor
