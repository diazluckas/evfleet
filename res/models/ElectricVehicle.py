...
from res.models.BatteryDegradation import severity as severity_fun
...
class ElectricVehicle:
    ...
    def battery_is_healthy(self, degradation=0.8):
        return True if self.battery_capacity/self.battery_capacity_nominal > degradation else False

    def degrade_battery(self, eta_table: np.ndarray, eta_model: NearestNeighbors, min_soc: float, max_soc: float,
                        nominal_cycles: int = 5500, set_model='aramis', consumed_energy=0, charge_energy=0):
        if set_model == 'aramis':
            eta_val = eta_fun(min_soc, max_soc, nominal_cycles, eta_table, eta_model)
            print(f'CAPACIDAAAAD {self.battery_capacity}')
            self.battery_capacity *= eta_val
            return eta_val
        else:
            capacity_res, severity_factor = severity_fun(min_soc, max_soc, charge_energy, consumed_energy, self.battery_capacity)
            self.battery_capacity = capacity_res
